...
def build_label_info():
    ...
    majority_label = freq.most_common(1)[0][0]

    return unique_labels, majority_label
...
def main():
    labels, majority_label = build_label_info()
    ...
    while True:
        ...
        random_pred = random.choice(labels)
        fine_tuned_pred = run_fine_tuned(user_text, labels, sampling_client, tokenizer)

        print("\nResults:")
        print(f"Majority baseline : {majority_label}")
        print(f"Random baseline   : {random_pred}")
        print(f"Fine-tuned model  : {fine_tuned_pred}\n")
